...
```
_config.yml               Configuración (nombre, correo, métricas de portada)
_data/
  es.yml / en.yml         Todos los textos de interfaz, en cada idioma
  people.yml              Equipo: una entrada por persona
  research.yml            Líneas de investigación
  projects.yml            Proyectos con financiamiento
...
_includes/                Piezas reutilizables (nav, pie, tarjeta de persona, íconos)
...
| Quiero… | Archivo |
|---|---|
| agregar una persona | `_data/people.yml` (copia una entrada; `group:` es `lead`, `postdoc` o `student`) |
| agregar un paper | `_data/publications.yml`, bloque `indexed` |
| agregar o editar un proyecto | `_data/projects.yml` |
...
Antes de editar aquí, actualiza `../informacion/INFORMACION-GRUPO.md`: ese es el
documento maestro y evita que el sitio y el material fuente se desincronicen.
...
## 7. Pendientes de este primer borrador
